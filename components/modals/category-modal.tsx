...
export function CategoryModal({ open, setOpen }: { open: boolean; setOpen: (val: boolean) => void }) {
  const [segment, setSegment] = useState("Pengeluaran");
        ...
        <Drawer.Content className="bg-canvas flex flex-col rounded-t-[32px] h-[85vh] mt-24 fixed bottom-0 left-0 right-0 z-[100] outline-none max-w-2xl mx-auto shadow-[0_-8px_40px_rgba(0,0,0,0.1)]">
          <div className="p-4 bg-canvas rounded-t-[32px] shrink-0 sticky top-0 z-10 border-b border-hairline">
            ...
            <div className="flex justify-between items-center px-2">
              <h2 className="text-[20px] font-bold text-ink">Buat Kategori Baru</h2>
              <button 
          ...
          <div className="p-6 overflow-y-auto space-y-6">
            <div>
              <label className="block text-[14px] font-semibold text-ink mb-2">Jenis Kategori</label>
              <div className="flex bg-surface-card rounded-full p-1 border border-hairline">
                <button 
                  onClick={() => setSegment("Pengeluaran")}
                  className={`flex-1 py-2 rounded-full font-bold text-[14px] transition-colors ${segment === "Pengeluaran" ? "bg-ink text-canvas shadow-sm" : "text-ink hover:bg-secondary-bg"}`}
                >Pengeluaran</button>
                <button 
                  onClick={() => setSegment("Tagihan")}
                  className={`flex-1 py-2 rounded-full font-bold text-[14px] transition-colors ${segment === "Tagihan" ? "bg-ink text-canvas shadow-sm" : "text-ink hover:bg-secondary-bg"}`}
                >Tagihan</button>
              </div>
            </div>

            <div>
              <label className="block text-[14px] font-semibold text-ink mb-2">Ikon</label>
            ...
            <div>
              <label className="block text-[14px] font-semibold text-ink mb-2">Nama Kategori</label>
              <input 
                type="text" 
            ...
            <div>
              <label className="block text-[14px] font-semibold text-ink mb-2">Limit Budget (Opsional)</label>
              ...
              <p className="text-[12px] text-mute mt-2">Batas maksimal pengeluaran bulanan untuk kategori ini.</p>
            </div>
          </div>

          <div className="p-6 bg-surface-card border-t border-hairline mt-auto">
            <button className="w-full h-14 bg-primary text-primary-foreground font-bold rounded-full text-[16px] hover:bg-primary-pressed transition-colors">
              Simpan Kategori
            </button>
          </div>
        </Drawer.Content>
