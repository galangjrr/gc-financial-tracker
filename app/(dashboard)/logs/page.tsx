  ...
  return (
    ...
    >
      <div className="space-y-6 w-full max-w-full min-w-0">
        ...
        <div className="flex items-center gap-3 p-4 bg-surface-card border border-hairline rounded-2xl">
        ...
        <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
          <div className="relative flex-1 max-w-md">
            <label htmlFor="search-log-input" className="sr-only">
              Cari log aktivitas
            </label>
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-ash pointer-events-none" aria-hidden="true" />
            <input
              id="search-log-input"
              type="search"
              ...
              className="w-full h-11 bg-canvas border border-hairline rounded-full pl-10 pr-10 text-xs md:text-sm font-medium text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-ink transition-all placeholder:text-ash"
            />
            {searchTerm && (
              <button
                type="button"
            ...
            <div className="space-y-3">
              {filteredLogs.map((log) => {
                ...
                return (
                  <div
                    key={log.id}
                    className="p-4 bg-canvas rounded-2xl border border-hairline/80 flex flex-col md:flex-row md:items-center justify-between gap-3 transition-all hover:border-hairline min-w-0 max-w-full overflow-hidden"
                  >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-secondary-bg text-ink flex items-center gap-1">
                            <User className="w-3 h-3 text-mute" aria-hidden="true" /> {log.actor_name}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center md:flex-col items-start md:items-end justify-between border-t md:border-t-0 pt-2 md:pt-0 border-hairline/50 text-[11px] text-mute shrink-0 min-w-0">
                      <div className="flex items-center gap-1.5 font-semibold text-ink">
                        <Clock className="w-3.5 h-3.5 text-mute" aria-hidden="true" />
                        <span>{formatTimestamp(log.created_at)}</span>
                      </div>
                      <div className="flex items-center gap-1 mt-1 font-medium">
                        <Smartphone className="w-3 h-3 text-mute" aria-hidden="true" />
