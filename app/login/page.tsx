...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      // Simpan preferensi sesi keluarga lokal dan arahkan langsung ke dashboard
      localStorage.setItem("gc_auth_email", email.trim());
      router.push("/dashboard");
    } catch {
      setErrorMessage("Terjadi kendala saat memproses login. Coba lagi.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full max-w-[420px] bg-canvas rounded-[28px] sm:rounded-[32px] p-6 sm:p-8 md:p-10 shadow-[0_8px_32px_rgba(0,0,0,0.06)] border border-hairline transition-all">
        ...
        <header className="text-center mb-6">
          ...
          <p className="text-xs sm:text-sm text-mute font-medium">
            Lanjutkan pencatatan keuangan keluarga hari ini
          </p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link 
                href="#" 
                className="text-xs font-semibold text-primary hover:text-primary-pressed transition-colors"
              >
                Lupa sandi?
              </Link>
            </div>
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full h-12 bg-ink text-canvas font-bold rounded-full text-sm hover:bg-ink-soft active:scale-[0.98] disabled:opacity-60 transition-all flex items-center justify-center gap-2 mt-2 shadow-sm focus-visible:ring-2 focus-visible:ring-ink focus-visible:ring-offset-2"
          >
            {isLoading ? (
